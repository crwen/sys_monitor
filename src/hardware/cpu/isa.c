...
static void mov_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    if (src_od->type == REG && dst_od->type == REG) {
        ...
        *(uint64_t *)dst = *(uint64_t *)src;
        ...
        // src: register
        // dst: virtual address
        cpu_write64bits_dram(va2pa(dst, cr), *(uint64_t *)src);
        next_rip(cr);
        reset_cflags(cr);
        return;
        ...
        // src: virtual address
        // dst: register
        *(uint64_t *)dst = cpu_read64bits_dram(va2pa(src, cr));
        next_rip(cr);
        reset_cflags(cr);
        return;
...
static void push_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    if (src_od->type == REG) {
        ...
        cpu_write64bits_dram(va2pa((cr->reg).rsp, cr), *(uint64_t *)src);
        next_rip(cr);
        reset_cflags(cr);
        return;
    }
...
static void pop_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    if (src_od->type == REG) {
        // src: register
        // dst: empty
        uint64_t old_val = cpu_read64bits_dram(va2pa((cr->reg).rsp, cr));
        (cr->reg).rsp = (cr->reg).rsp + 8;
...
static void call_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    (cr->reg).rsp = (cr->reg).rsp - 8;
    cpu_write64bits_dram(va2pa((cr->reg).rsp, cr), cr->rip + sizeof(char) * MAX_INSTRUCTION_CHAR);
...
static void ret_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    // dst: empty
    // pop rsp
    uint64_t ret_addr = cpu_read64bits_dram(va2pa((cr->reg).rsp, cr));
    (cr->reg).rsp = (cr->reg).rsp + 8;
...
static void cmp_handler(od_t *src_od, od_t *dst_od, core_t *cr) {
    ...
    if (src_od->type == IMM && dst_od->type >= MEM_IMM) {
        ...
        // dst: mem 
        // dst = dst - src
        uint64_t dval = cpu_read64bits_dram(va2pa(dst, cr));
