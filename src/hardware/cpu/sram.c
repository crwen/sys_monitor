...
/*
    interface of I/O Bus: read and write between the SRMA cache and DRAM memory
*/

// read data block from paddr
static void read_sram_cacheline(address_t paddr, uint8_t *block) {
    uint64_t size64 = sizeof(uint64_t);

    uint64_t dram_base = ((paddr.paddr_value >> SRAM_CACHE_OFFSET_LENGTH)) << SRAM_CACHE_OFFSET_LENGTH;
...
static void write_sram_cacheline(address_t paddr, uint8_t *block) {
    ...
    for (int i = 0; i < ((1 << SRAM_CACHE_OFFSET_LENGTH) / size64); i ++) {
        ...
        // write to memory
        write64bits_dram(dram_addr, *(uint64_t *)block_addr);
